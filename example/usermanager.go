...
func mail(to string , msg string , subject string)error{
	...
	d := gomail.NewDialer("SMTP ADDRESS", 587, "EMAIL NAME", "EMAIL PASS")
				...
				if (err1 != nil){
					...
					if (err1 != nil) {
						...
						if answer {
							...
							msg := "Confirmation Links :   \r\n\r\n  http://ADDRESS:3000/confirm?code=" + confirmcode + "&user=" + user[0] + ".\r\n"
			...
			if answer {
				...
				if (err != nil) {
					...
					if (orginalemail == email[0]) {
						...
						fmt.Fprintf(w, "Please Check Your Mail For Reset Password Link.")
						subject := "Rest Password"
						msg := "Reset Password Link :   \r\n\r\n  http://ADDRESS:3000/resetpass2?email=" + email[0] + "\r\n"
						err := mail(email[0],msg , subject)
						if err != nil{
							log.Fatal(err)
							fmt.Fprintf(w, "0")
						...
						fmt.Fprintf(w, "Email Is Wrong With Orginal Email In Database \r\n")
					}
				}
...
func resetpass2(w http.ResponseWriter, req *http.Request, userstate go_user_manage.IUserState) {
		...
		if (email != nil) && (user != nil) {
			...
			if (err != nil) {
				...
				if (orginalemail == email[0]) {
					...
					fmt.Fprintf(w, "Please Check Your Mail For New Password .")
...
func changepass(w http.ResponseWriter, req *http.Request, userstate go_user_manage.IUserState) {
	...
	fmt.Println("method:", req.Method) //get request method
	if req.Method == "GET" {
		fmt.Fprintf(w, "Change Password User Api") // write data to response
	} else {
		w.Header().Set("Access-Control-Allow-Origin", "*")
		req.ParseForm()
...
func makeHandler(fn func(http.ResponseWriter, *http.Request, go_user_manage.IUserState),userstate go_user_manage.IUserState) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		path := []string{"/register","/confirm" ,"/login","/logout","/resetpass","/resetpass2","/changepass"}
		m := (r.URL.Path)
		c := 0
		for _,i := range path{
			if m == i {
...
func main() {
	...
	perm, err := go_user_manage.New(0, "", "REDIS PASSWORD")
	if err != nil {
		log.Fatalln(err)
	}
	...
	http.HandleFunc("/resetpass2", makeHandler(resetpass2,userstate))
